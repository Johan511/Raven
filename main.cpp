...
//
uint32_t DecodeHexBuffer(_In_z_ const char *HexBuffer, _In_ uint32_t OutBufferLen,
...
void RunClient(_In_ int argc, _In_reads_(argc) _Null_terminated_ char *argv[]) {
    ...
    //
    if (QUIC_FAILED(Status = MsQuic->ConnectionOpen(Registration, ClientConnectionCallback, NULL,
                                                    &Connection))) {
    ...
    if (QUIC_FAILED(Status = MsQuic->ConnectionStart(
                        Connection, Configuration, QUIC_ADDRESS_FAMILY_UNSPEC, Target, UdpPort))) {
