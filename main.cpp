...
//
const QUIC_REGISTRATION_CONFIG RegConfig = { "quicsample", QUIC_EXECUTION_PROFILE_TYPE_REAL_TIME };
...
//
BOOLEAN
GetFlag(_In_ int argc, _In_reads_(argc) _Null_terminated_ char* argv[], _In_z_ const char* name)
{
...
ServerLoadConfiguration(_In_ int argc, _In_reads_(argc) _Null_terminated_ char* argv[])
{
    ...
    if ((Cert = GetValue(argc, argv, "cert_hash")) != NULL)
    {
        ...
        DecodeHexBuffer(Cert, sizeof(Config.CertHash.ShaHash), Config.CertHash.ShaHash);
...
void RunServer(_In_ int argc, _In_reads_(argc) _Null_terminated_ char* argv[])
{
    ...
    //
    if (QUIC_FAILED(Status = MsQuic->ListenerOpen(Registration, ServerListenerCallback,
                                                  NULL, &Listener)))
    {
...
void ClientSend(_In_ HQUIC Connection)
{
    ...
    //
    if (QUIC_FAILED(Status = MsQuic->StreamOpen(Connection, QUIC_STREAM_OPEN_FLAG_NONE,
...
void RunClient(_In_ int argc, _In_reads_(argc) _Null_terminated_ char* argv[])
{
    ...
    //
    if (QUIC_FAILED(Status = MsQuic->ConnectionOpen(Registration, ClientConnectionCallback,
                                                    NULL, &Connection)))
    {
    ...
    if ((ResumptionTicketString = GetValue(argc, argv, "ticket")) != NULL)
    {
        ...
        (uint16_t)DecodeHexBuffer(ResumptionTicketString,
                                  sizeof(ResumptionTicket), ResumptionTicket);
