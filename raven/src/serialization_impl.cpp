...
mock_serialize(const rvn::DeliveryTimeoutParameter &parameter) {
  ...
  std::uint64_t parameterLength =
      ds::quic_var_int(parameter.timeout_.count()).size();
  parameterTotalLen += mock_serialize<ds::quic_var_int>(parameterLength);
  parameterTotalLen +=
      mock_serialize<ds::quic_var_int>(parameter.timeout_.count());
  return parameterTotalLen;
}

[[nodiscard]] serialize_return_t
mock_serialize(const rvn::Parameter &parameter) {
  return std::visit([](const auto &param) { return mock_serialize(param); },
                    parameter.parameter_);
}

serialize_return_t serialize(ds::chunk &c,
                             const rvn::DeliveryTimeoutParameter &parameter) {
  std::uint64_t parameterTotalLen = 0;
  parameterTotalLen += serialize<ds::quic_var_int>(
      c, utils::to_underlying(ParameterType::DeliveryTimeout));
  std::uint64_t parameterLength =
      ds::quic_var_int(parameter.timeout_.count()).size();
...
serialize(ds::chunk &c, const rvn::ClientSetupMessage &clientSetupMessage) {
  ...
  // we need to find out length of the message we would be serializing
  {
    msgLen += mock_serialize<ds::quic_var_int>(
        clientSetupMessage.supportedVersions_.size());
  ...
  // Header
  headerLen += serialize<ds::quic_var_int>(
      c, utils::to_underlying(MoQtMessageType::CLIENT_SETUP));
...
serialize(ds::chunk &c, const rvn::ServerSetupMessage &serverSetupMessage) {
  ...
  {
    msgLen +=
        mock_serialize<ds::quic_var_int>(serverSetupMessage.selectedVersion_);
    msgLen +=
  ...
  std::uint64_t headerLen = 0;
  headerLen += serialize<ds::quic_var_int>(
      c, utils::to_underlying(MoQtMessageType::SERVER_SETUP));
  headerLen += serialize<ds::quic_var_int>(c, msgLen);
...
static serialize_return_t
mock_serialize(const rvn::SubscribeMessage &subscribeMessage) {
  std::uint64_t msgLen = 0;
  msgLen += mock_serialize<ds::quic_var_int>(subscribeMessage.subscribeId_);
  msgLen +=
      mock_serialize<ds::quic_var_int>(subscribeMessage.trackAlias_.get());
  ...
  msgLen +=
      mock_serialize<ds::quic_var_int>(subscribeMessage.trackName_.size());
  ...
  if (subscribeMessage.start_.has_value()) {
    ...
    msgLen += mock_serialize<ds::quic_var_int>(
        subscribeMessage.start_->object_.get());
  }
  ...
  msgLen +=
      mock_serialize<ds::quic_var_int>(subscribeMessage.parameters_.size());
...
void serialize_without_header(ds::chunk &c,
  ...
  c.append(subscribeMessage.trackName_.data(),
           subscribeMessage.trackName_.size());

  serialize<std::uint8_t>(c, subscribeMessage.subscriberPriority_);
  serialize<std::uint8_t>(c, subscribeMessage.groupOrder_);
  serialize<ds::quic_var_int>(
      c, utils::to_underlying(subscribeMessage.filterType_));
...
serialize_return_t serialize(ds::chunk &c,
  ...
  std::uint64_t headerLen = 0;
  headerLen += serialize<ds::quic_var_int>(
      c, utils::to_underlying(MoQtMessageType::SUBSCRIBE));
  headerLen += serialize<ds::quic_var_int>(c, msgLen);
...
serialize(ds::chunk &c,
  ...
  {
    msgLen +=
        mock_serialize<ds::quic_var_int>(subscribeErrorMessage.subscribeId_);
    msgLen +=
        mock_serialize<ds::quic_var_int>(subscribeErrorMessage.errorCode_);
    msgLen += mock_serialize<ds::quic_var_int>(
        subscribeErrorMessage.reasonPhrase_.size());
    msgLen += subscribeErrorMessage.reasonPhrase_.size();
    msgLen +=
        mock_serialize<ds::quic_var_int>(subscribeErrorMessage.trackAlias_);
  }

  std::uint64_t headerLen = 0;
  // Header
  headerLen += serialize<ds::quic_var_int>(
      c, utils::to_underlying(MoQtMessageType::SUBSCRIBE_ERROR));
  headerLen += serialize<ds::quic_var_int>(c, msgLen);
...
serialize_return_t
serialize(ds::chunk &c,
          const rvn::BatchSubscribeMessage &batchSubscribeMessage) {
  std::uint64_t msgLen = 0;
  // we need to find out length of the message we would be serializing
  {
    msgLen += mock_serialize<ds::quic_var_int>(
    ...
    msgLen += mock_serialize<ds::quic_var_int>(
        batchSubscribeMessage.subscriptions_.size());
  ...
  // Header
  headerLen += serialize<ds::quic_var_int>(
      c, utils::to_underlying(MoQtMessageType::BATCH_SUBSCRIBE));
  headerLen += serialize<ds::quic_var_int>(c, msgLen);

  serialize<ds::quic_var_int>(
      c, batchSubscribeMessage.trackNamespacePrefix_.size());
  for (const auto &ns : batchSubscribeMessage.trackNamespacePrefix_) {
    serialize<ds::quic_var_int>(c, ns.size());
    c.append(ns.data(), ns.size());
  }
