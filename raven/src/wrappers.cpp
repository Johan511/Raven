...
unique_registration::unique_registration(
    const QUIC_API_TABLE *tbl_, const QUIC_REGISTRATION_CONFIG *RegConfig)
...
unique_connection::unique_connection(const QUIC_API_TABLE *tbl_,
    ...
    : unique_handler2(tbl_->ConnectionOpen, tbl_->ConnectionClose,
                      tbl_->ConnectionStart) {
  if (QUIC_FAILED(open_handler(openParams.registration, openParams.connectionCb,
                               openParams.context)))
    throw std::runtime_error("ConnectionOpenFailure");

  if (QUIC_FAILED(start_handler(startParams.Configuration, startParams.Family,
                                startParams.ServerName,
                                startParams.ServerPort)))
