...
MOQTClient::MOQTClient(
  ...
  QUIC_STATUS status = tbl->SetParam(
      nullptr, QUIC_PARAM_GLOBAL_EXECUTION_CONFIG, execConfigLen, execConfig);
...
void MOQTClient::start_connection(QUIC_ADDRESS_FAMILY Family,
  ...
  rvn::utils::ASSERT_LOG_THROW(
      secondaryCounter == full_sec_counter_value(), "secondaryCounter ",
      secondaryCounter, " full_sec_counter_value() ", full_sec_counter_value());

  reg = rvn::unique_registration(tbl.get(), regConfig);
  configuration = rvn::unique_configuration(
      tbl.get(),
      {reg.get(), AlpnBuffers, AlpnBufferCount, Settings, SettingsSize, this},
      {CredConfig});

  // enable critical section
  //            => no RAII because if emplace fails, we don't want connections
  //            to be accepted and fault elsewhere
  auto connection = rvn::unique_connection(
      tbl.get(), {reg.get(), MOQT::connection_cb_wrapper, this},
      {configuration.get(), Family, ServerName, ServerPort});

  // connection state is optional
  connectionState =
      std::make_shared<ConnectionState>(std::move(connection), *this);
