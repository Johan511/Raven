...
///////////////////////////////////////////////////////////////////////////////////////////////
// Serialize and deserialize trivial types (std::uint8_t, std::uint16_t,
...
serialize_return_t serialize_trivial(ds::chunk &c, const auto &value,
                                     ToEndianness = network_endian) {
  static_assert(
      std::is_same_v<T, std::uint8_t> || std::is_same_v<T, std::uint16_t> ||
          std::is_same_v<T, std::uint32_t> || std::is_same_v<T, std::uint64_t>,
  ...
  if (std::is_same_v<ToEndianness, NativeEndian> ||
...
/*
   ...
   as described in ([RFC9000], Section 16)
*/
