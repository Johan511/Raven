...
public:
  ...
  chunk(std::uint64_t maxSize)
      : currSize_{0}, maxSize_{maxSize},
        ...
        data_{static_cast<std::uint8_t *>(
            malloc(sizeof(std::uint8_t) * maxSize))} {
    if (data_ == nullptr) {
      currSize_ = 0;
      maxSize_ = 0;
      throw std::bad_alloc();
  ...
  ~chunk() noexcept { free(data_); }

  std::uint64_t size() const noexcept { return currSize_; }

  std::uint64_t capacity() const noexcept { return maxSize_; }
...
public:
  ...
  ChunkSpan(const chunk &chunk, std::uint64_t beginOffset,
  ...
  std::uint64_t size() const noexcept { return endOffset_ - beginOffset_; }
