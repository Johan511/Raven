...
x (i): -> quic_var_int
Indicates that x holds an integer value using the variable-length encoding as
described in ([RFC9000], Section 16)

x (..):
Indicates that x can be any length including zero bits long. Values in this
format always end on a byte boundary.

[x (L)]: -> std::optional<quic_var_int>
Indicates that x is optional and has a length of L

x (L) ...: -> Container<T> -> serialize all of them without any number of
elements tag Indicates that x is repeated zero or more times and that each
instance has a length of L

This document extends the RFC9000 syntax and with the additional field types:

x (b): -> std::string or ds::chunk, serialized as quic_var_int then memcpy (do
we care about endianess?) Indicates that x consists of a variable length integer
encoding as described in ([RFC9000], Section 16), followed by that many bytes of
binary data
