...
// if callback is to be called after t, we will call it before t + jitter ( and
// after t - jitter (do we need t - jitter condition) )
  ...
  void poll() {
    ...
    if ((lastProcessedTime_ - currMsTime).count() >= Jitter * NumSlots)
      ...
      std::uint64_t beginIndex =
          (lastProcessedTime_.count() / Jitter) % NumSlots;
      std::uint64_t endIndex = (currMsTime.count() / Jitter) % NumSlots;

      while (beginIndex != endIndex) {
        timers_[beginIndex].write([&](std::vector<TimerEvent> &slot) {
          for (auto &timerEvent : slot) {
            timerEvent.callback_(timerEvent.index_);
          }
...
public:
  ...
  TimerIndex add_timer(std::chrono::milliseconds duration,
                       Callback &&callback) {
    if (threadJoinFlag_)
      return -1;
    ...
    if ((lastProcessedTime_ - currMsTime).count() >= Jitter * NumSlots)
      // events have not been polled for a very long time, need to do something
      // about it
      clear_all();
    ...
    timers_[slotIndex].write([&](auto &slot) {
  ...
  Timer()
    ...
    // cannot be part of initializer list because we use poll function (cant use
    // non static members before construction)
    pollThread_ = std::jthread([this]() {
      while (true) {
