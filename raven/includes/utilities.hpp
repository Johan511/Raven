...
#define LOGE(...)                                                              \
  rvn::utils::LOG(std::source_location::current(),                             \
                  "LOGGING UNEXPECTED STATE: ", __VA_ARGS__)
...
#define DECLARE_SINGLETON(ClassName)                                           \
  ...
  public:                                                                      \
    ...
    ClassName *get_instance() { return instance; }                             \
    ClassName &operator*() { return *instance; }                               \
    const ClassName &operator*() const { return *instance; }                   \
    ClassName *operator->() { return instance; }                               \
    const ClassName *operator->() const { return instance; }                   \
  };
...
static void ASSERT_LOG_THROW([[maybe_unused]] bool assertion,
...
static inline bool xnor(bool a, bool b) { return (a && b) || (!a && !b); }
...
static inline void thread_set_max_priority(std::thread &th) {
  ...
  if (int result = pthread_setschedparam(th.native_handle(), policy, &sch)) {
    std::cerr << "Failed to set Thread Priority, result= " << result
              << std::endl;
    exit(1);
  }
}
...
template <typename T>
bool optional_equality(const std::optional<T> &lhs,
...
struct MOQTUtilities {
  static void check_setting_assertions(QUIC_SETTINGS *Settings_,
