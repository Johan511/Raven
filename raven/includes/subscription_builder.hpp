...
class SubscriptionBuilder {
  ...
  */
  ...
  constexpr std::uint64_t all_elements_set() { return 0b111111; }
...
public:
  SubscriptionBuilder() { set_defaults(); }
  ...
  SubscriptionBuilder &
  set_track_namespace(std::vector<std::string> trackNamespace) {
    subscribeMessage_.trackNamespace_ = std::move(trackNamespace);
    setElementsCounter_ |=
        (1 << utils::to_underlying(SetElements::TrackNamespace));
    return *this;
  }
  ...
  SubscriptionBuilder &
  set_subscriber_priority(std::uint64_t subscriberPriority) {
  ...
  class Filter {
    ...
    struct LatestPerGroupInTrack {
      static constexpr auto filterType =
          SubscribeFilterType::LatestPerGroupInTrack;
    };
  ...
  SubscriptionBuilder &set_data_range(Filter::AbsoluteStart f,
                                      GroupObjectPair start) {
    subscribeMessage_.filterType_ = f.filterType;
