...
class unique_registration : public detail::unique_handler1<
                                decltype(QUIC_API_TABLE::RegistrationOpen),
                                decltype(QUIC_API_TABLE::RegistrationClose)> {
public:
  unique_registration(const QUIC_API_TABLE *tbl_,
...
class unique_connection
    : public detail::unique_handler2<decltype(QUIC_API_TABLE::ConnectionOpen),
...
class QUIC_BUFFERDeleter {
  ...
  // QUIC_BUFFER provided by MsQuic, because of lifetime management issues of
  // the MsQuic buffer when multireceive is enabled
  [[maybe_unused]] HQUIC streamHandle_;
  [[maybe_unused]] QUIC_STREAM_RECEIVE_COMPLETE_FN
      streamReceiveCompletefunction_;
...
public:
  ...
  QUIC_BUFFERDeleter(
      HQUIC streamHandle,
      QUIC_STREAM_RECEIVE_COMPLETE_FN streamReceiveCompletefunction)
