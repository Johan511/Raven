...
int main() {
    ...
    thread = std::thread([&]() {
      for (std::uint64_t i = 0; i < numTimersPerThread; i++) {
        timer.add_timer(
            ...
            [currTime = SteadyClock::now(), &totalJitter,
             &numFinishedTimers](std::uint64_t) {
  ...
  while (numFinishedTimers.load(std::memory_order_relaxed) <
         numThreads * numTimersPerThread)
    std::this_thread::sleep_for(std::chrono::milliseconds(1));

  std::cout << "Average jitter: "
            << totalJitter / (numTimersPerThread * numThreads) << std::endl;
