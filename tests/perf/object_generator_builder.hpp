...
class ObjectGeneratorFactory {
  ...
  static std::uint64_t
  get_size_of_object(std::uint64_t bitRate,
                     std::chrono::milliseconds msBetweenObjects) {
    std::uint64_t bytesPerSecond = bitRate / 8;
    std::uint64_t bytesPerObject =
        bytesPerSecond * msBetweenObjects.count() / 1000;
  ...
  static std::string generate_object(std::uint64_t objectSize,
    ...
    std::memcpy(object.data() + sizeof(currTime) + sizeof(layerId), &objectId,
                sizeof(objectId));
...
public:
  ...
  std::vector<std::thread> create(LayerGranularity layerGranularity,
    ...
    static const std::vector<std::string> trackNamespace{
        "namespace1", "namespace2", "namespace3"};

    const auto trackGranularityGeneratorTask = [this, baseBitRate,
                                                numObjectsPerLayer,
                                                msBetweenObjects](
                                                   std::uint64_t layerIdx) {
      auto trackHandle =
          dataManager_
      ...
      auto groupHandle =
          trackHandle->add_group(rvn::GroupId(0), priority, msBetweenObjects)
              .lock();

      std::optional<rvn::SubgroupHandle> subGroupHandleOpt =
          groupHandle->add_open_ended_subgroup();

      for (std::uint64_t objectId = 0; objectId < numObjectsPerLayer;
           ++objectId) {
    ...
    const auto groupGranularityGeneratorTask =
        [baseBitRate, numObjectsPerLayer,
         msBetweenObjects](std::shared_ptr<rvn::TrackHandle> trackHandle,
                           std::uint64_t layerIdx) {
          std::uint64_t bitRate = baseBitRate << layerIdx;
          std::uint64_t objectSize =
              get_size_of_object(bitRate, msBetweenObjects);
          rvn::PublisherPriority priority(layerIdx);

          auto groupHandle = trackHandle
                                 ->add_group(rvn::GroupId(layerIdx), priority,
                                             msBetweenObjects)
                                 .lock();
          ...
          for (std::uint64_t objectId = 0; objectId < numObjectsPerLayer;
               ++objectId) {
            std::string object =
                generate_object(objectSize, layerIdx, objectId);
            subGroupHandleOpt->add_object(std::move(object));
            subGroupHandleOpt.emplace(
                subGroupHandleOpt->cap_and_next().value());
            std::this_thread::sleep_for(msBetweenObjects);
          }
        };
