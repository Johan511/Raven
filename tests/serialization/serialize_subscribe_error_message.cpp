...
void test_serialize_subscribe_error() {
  ...
  for (std::size_t i = 0; i < c.size(); i++)
    utils::ASSERT_LOG_THROW(c[i] == expectedSerialization[i],
                            "Mismatch at index: ", i, "\n",
  ...
  utils::ASSERT_LOG_THROW(
      header.messageType_ == MoQtMessageType::SUBSCRIBE_ERROR,
      "Message type mismatch\n",
      "Expected: ", utils::to_underlying(MoQtMessageType::SUBSCRIBE_ERROR),
