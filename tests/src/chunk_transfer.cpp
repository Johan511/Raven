...
int main() {
  ...
  if (fork()) {
    ...
    std::array groupHandles = {
        trackHandle.lock()->add_group(
            GroupId(0), PublisherPriority(3),
            {}), // high priorty means would be sent first
    ...
    std::for_each(
        std::execution::par, groupHandles.begin(), groupHandles.end(),
        [](std::weak_ptr<GroupHandle> &groupHandle) {
          std::uint64_t groupId = groupHandle.lock()->groupIdentifier_.groupId_;

          auto groupHandleSharedPtr = groupHandle.lock();
          for (std::size_t idx = 0; idx < numObjects; ++idx) {
            std::string object = "Group_" + std::to_string(groupId) + "_" +
                                 "ID_" + std::to_string(idx);
            groupHandleSharedPtr->add_subgroup(1).add_object(std::move(object));
          }
        });
    {
    ...
    subscriptionBuilder.set_data_range(
        SubscriptionBuilder::Filter::absoluteStart, {GroupId(0), ObjectId(0)});
      ...
      streamConsumerThreads.emplace_back(
          [](MOQTClient::DataStreamUserHandle &&dataStreamUserHandle) {
            ...
            for (;;) {
              ...
              if (streamHeaderSubgroupObject.objectId_ ==
                  ObjectId(numObjects - 1))
                break;
            }
          },
